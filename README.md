...
This part of the program uses the website KeepTradeCut.com, which is basically a website that creates a crowdsourced numeric value for all dynasty assets (players and draft picks). I used the Selenium library to add together the value of all of the assets on each individual team, and then inserted those values into a table in the project database. I then used Windows Task Scheduler to run the program on the first day of each month, so that over time the league and myself will be able to see how teams change in value. I have tried to create visual graphics to display the information to the rest of my league more clearly. 
